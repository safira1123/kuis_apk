...
import { BookOpen, Eye, X, CheckCircle, Trash2 } from 'lucide-react';
...
export default function StudentMaterialsTab() {
  ...
  const handleDeleteMaterial = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
  ...
  const progressPercentage = materials.length > 0 ? Math.round((readCount / materials.length) * 100) : 0;
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-primary-foreground">{progressPercentage}%</span>
        </div>
      </div>
      ...
      {materials.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
          {materials.map((mat) => (
            <div
              key={mat.id}
              className={`card-soft p-4 cursor-pointer hover:shadow-soft-green transition-all duration-200 slide-up group relative ${mat.isRead ? 'border-pastel-green border-opacity-50' : ''}`}
